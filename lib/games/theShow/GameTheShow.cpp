...
std::vector<std::pair<std::string, std::pair<std::pair<int, int>, std::pair<int, int>>>> arcade::GameTheShow::displayLevel4()
{
    ...
    if (elapsed.count() > 400)
        isMoving = false;
    entities.emplace_back("assets/theShow/chateau.png", std::make_pair(std::make_pair(0, 0), std::make_pair(1080, 720)));
    if (isMoving == true) {
        elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(now - lastMoveTime);
        if (elapsed.count() > 200) {
            moveIndex = (moveIndex + 1) % 3;
    ...
    if (isEnnemiAttack == true) {
        ...
        if (elapsed.count() > 150) {
            ennemiMoveIndex = (ennemiMoveIndex + 1) % 6;
            lastEnnemiMoveTime = now;
            if (ennemiMoveIndex == 5) {
                isEnnemiAttack = false;
                ennemiMoveIndex = 0;
                life--;
                if (life == 0) {
        ...
        if (isEnnemiRight == true) {
            if (ennemiMoveIndex == 0)
                entities.emplace_back("assets/theShow/JohnIdle.png", std::make_pair(ennemiPos, std::make_pair(120, 150)));
            else if (ennemiMoveIndex == 1)
                entities.emplace_back("assets/theShow/JohnAttack1.png", std::make_pair(ennemiPos, std::make_pair(150, 150)));
            else if (ennemiMoveIndex == 2)
                entities.emplace_back("assets/theShow/JohnAttack2.png", std::make_pair(ennemiPos, std::make_pair(120, 150)));
            else if (ennemiMoveIndex == 3)
                entities.emplace_back("assets/theShow/JohnAttack3.png", std::make_pair(ennemiPos, std::make_pair(120, 150)));
            else if (ennemiMoveIndex == 4)
                entities.emplace_back("assets/theShow/JohnAttack4.png", std::make_pair(ennemiPos, std::make_pair(120, 150)));
        } else {
            if (ennemiMoveIndex == 0)
                entities.emplace_back("assets/theShow/JohnIdle2.png", std::make_pair(ennemiPos, std::make_pair(120, 150)));
            else if (ennemiMoveIndex == 1)
                entities.emplace_back("assets/theShow/JohnAttack12.png", std::make_pair(ennemiPos, std::make_pair(150, 150)));
            else if (ennemiMoveIndex == 2)
                entities.emplace_back("assets/theShow/JohnAttack22.png", std::make_pair(ennemiPos, std::make_pair(120, 150)));
            else if (ennemiMoveIndex == 3)
                entities.emplace_back("assets/theShow/JohnAttack32.png", std::make_pair(ennemiPos, std::make_pair(120, 150)));
            else if (ennemiMoveIndex == 4)
                entities.emplace_back("assets/theShow/JohnAttack42.png", std::make_pair(ennemiPos, std::make_pair(120, 150)));
        }
    } else if (isEnnemiRight == true)
        entities.emplace_back("assets/theShow/JohnIdle.png", std::make_pair(ennemiPos, std::make_pair(120, 150)));
    else
...
{
    std::vector<std::pair<std::string, std::pair<std::pair<int, int>, std::pair<int, int>>>> entities;
    entities.emplace_back("assets/theShow/merci.png", std::make_pair(std::make_pair(0, 0), std::make_pair(1080, 720)));
    return entities;
}
